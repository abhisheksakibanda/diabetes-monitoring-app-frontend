...
const Trends = () => {
  ...
  useEffect(() => {
    if (isFocused) {
      // Fetch userDates
      console.log("UserID when fetching userDates: ", user);
      axios.get(`${BASE_URL}/api/userDates`, {
        params: {
          userId: user?.user?.uid,
        },
      }).then(res => {
        console.log("Dates: ", res);
        setUserDates(res.data.slice(0, 7));
        // Now you can fetch dataForTrends using mealDate
        axios.get(`${BASE_URL}/api/getUserData`, {
          params: {
            userId: user?.user?.uid,
          },
        }).then(dataRes => {
    ...
    datasets: [
      {
        data: userDates.map(date => {
          const matchingData = userMealData.find(item => item.mealDate === date.mealDate && item.mealType === "Breakfast");
